...
//            cv::imshow(name, image);
    ...
    void toSketch(cv::Mat const &_image, cv::Mat const &pattern, cv::Mat &sketch,
        ...
        if (profile.use_sketch_texture) {
            texture = pattern;// cv::imread(profile.sketch_texture_path);
        ...
        cv::Mat kernel = cv::Mat::ones(cv::Size(profile.edge_dilate_size, profile.edge_dilate_size),
                                       CV_8U);
...
//    cv::Mat toSketch(cv::Mat const &_image, cv::Mat const &pattern, ConvertProfile const &profile,
//                     Error &error) {
//        cv::Mat image;
//        if (_image.channels() == 4) {
//            cv::cvtColor(_image, image, cv::COLOR_RGBA2BGR);
...
//        // 3. 自适应二值化
//        cv::Mat threshold;
//        cv::adaptiveThreshold(gray, threshold, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C,
...
//
//        // 5. 纹理生成 (假设纹理比图片小)
//        cv::Mat texture = pattern;// cv::imread(RESOURCE + "/素描纹理.png");
//        cv::detailEnhance(texture, texture);
//        cv::cvtColor(texture, texture, cv::COLOR_BGR2GRAY);
//        texture.convertTo(texture, CV_32F);
//        cv::Mat textureMap, tmpTexture, tmpSubTexture;
//        cv::copyMakeBorder(texture, texture, 0, std::max(gray.rows - texture.rows, 0),
//                           0, std::max(gray.cols - texture.cols, 0), cv::BORDER_WRAP);
//        tmpTexture = texture.clone();
//        cv::flip(tmpTexture, textureMap, 1);
//        texture += textureMap;
//        cv::flip(tmpTexture, textureMap, 0);
